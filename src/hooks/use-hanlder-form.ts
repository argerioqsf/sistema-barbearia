...
export const useHandlerForm = (
    ...
    for (let i = 0; i < sections?.length; i++) {
      ...
      for (let j = 0; j < boxs.length; j++) {
        ...
        for (let k = 0; k < fields.length; k++) {
          ...
          if (
            fields[k].type === "text" ||
            (fields[k].type === "password" && fields[k].type === "date")
          ) {
            validation = z.string().min(fields[k].roles?.minCaracters ?? 1, {
              message:
                fields[k].messageError ??
                `O campo ${fields[k].label} é requerido`,
            });
  ...
  const NewSchema = z.object(arraySchema);
