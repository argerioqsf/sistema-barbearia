...
export async function applyCoupon(
  ...
  try {
    const dto = ApplyCouponDTO.parse(input)
    const gateway = deps?.gateway ?? new SalesGateway()
    const sale = await gateway.applyCoupon(dto)
...
export async function removeCoupon(
  ...
  try {
    const dto = RemoveCouponDTO.parse(input)
    const gateway = deps?.gateway ?? new SalesGateway()
    const sale = await gateway.removeCoupon(dto)
    return ok(sale.toDTO())
  } catch (error) {
    return err(normalizeError(error))
  }
